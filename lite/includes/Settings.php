...
class Settings {
						...
						foreach ( $section['fields'] as $field ) {
							if ( isset( $field['id'] ) && $field['id'] && isset( $field['title'] ) ) {
								...
								add_settings_field( $field['id'], $title, [
									$this,
									'generate_setting',
								], $page_name, $section['section_id'], [ 'section' => $section, 'field' => $field ] );
							}
						}
					}
				}
	...
	public function generate_checkboxes_field( $args ) {
		...
		foreach ( $args['choices'] as $value => $text ) {
			...
			echo sprintf( '<li class="mb-3"><label><input type="checkbox" name="%s[]" id="%s" value="%s" class="%s" %s> %s</label></li>', $args['name'], $field_id, $value, $args['class'] . ' form-checkbox text-indigo-600', $checked, $text );
		}
